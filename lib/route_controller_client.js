...
RouteController.prototype._runRoute = function (route, url, done) {
  ...
  // create stack of 'condition' hook and layout prerendering
  self._conditionExecuted = false;
  ...
  var conditionHook = !!this._collectHooks('condition').length
    ? this._collectHooks('condition')
    : function(req, res, next){ next(); }; // default

  var onConditionExecuted = function (req, res, next) {
    self._conditionExecuted = true;
    next();
  };
  ...
  if (useLayoutPrerendering) {
    onBeforeRunStack.append(layoutPrerendering, conditionHook, onConditionExecuted, {where: 'client'});
  }
  else {
    onBeforeRunStack.append(conditionHook, onConditionExecuted, layoutPrerendering, {where: 'client'});
  }
  ...
  Deps.afterFlush(function() {
    Iron.utils.warn(self._rendered || self.isStopped || !self._conditionExecuted,
      "Route dispatch never rendered. Did you forget to call this.next() in an condition/onBeforeAction?");
  });
